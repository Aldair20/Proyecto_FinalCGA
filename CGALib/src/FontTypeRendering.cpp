...
void FontTypeRendering::Initialize() {
	...
	if (FT_New_Face(ft_lib, "../Fonts/RetroGaming.ttf", 0, &face) != 0) {
		std::cerr << "Unable to load RetroGaming.ttf\n";
		cleanup();
		return exit(-1);
	}
 ...
 */
void FontTypeRendering::render(const std::string &str, float x, float y, float r, float g, float b, int size) {
